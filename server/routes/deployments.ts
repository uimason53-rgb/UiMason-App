...
// One-click deployment endpoints — Vercel & Netlify
// Backend proxies to provider APIs; stores deployment history
// ─────────────────────────────────────────────────────────────
...
const getCachedValidation = (cacheKey: string) => {
  const cacheEntry = tokenValidationCache.get(cacheKey);
...
const validateVercelToken = async (token: string, teamId?: string) => {
  ...
  if (getCachedValidation(cacheKey)) {
    return;
  }
...
// ── Helper: Create zip buffer from files ────────────────────
const createZipFromFiles = (files: Array<{ path: string; content: string }>): Buffer => {
  // Simple zip implementation — creates a valid zip archive
  const localFileHeader = (name: string, data: Buffer): Buffer => {
    const nameBuf = Buffer.from(name, "utf-8");
    const header = Buffer.alloc(30 + nameBuf.length);
    let offset = 0;
    header.writeUInt32LE(0x04034b50, offset); offset += 4; // signature
    header.writeUInt16LE(20, offset); offset += 2; // version needed
    header.writeUInt16LE(0, offset); offset += 2; // flags
    header.writeUInt16LE(0, offset); offset += 2; // compression (stored)
    header.writeUInt16LE(0, offset); offset += 2; // mod time
    header.writeUInt16LE(0, offset); offset += 2; // mod date
    // CRC-32 of data
    const crc = crc32(data);
    header.writeUInt32LE(crc, offset); offset += 4;
    header.writeUInt32LE(data.length, offset); offset += 4; // compressed size
    header.writeUInt32LE(data.length, offset); offset += 4; // uncompressed size
    header.writeUInt16LE(nameBuf.length, offset); offset += 2; // name length
    header.writeUInt16LE(0, offset); offset += 2; // extra field length
  ...
  for (const file of files) {
    ...
    const central = centralDirEntry(file.path, data, localOffset);
    centralEntries.push(central);
    localOffset += local.length;
  }

  const cdBuffer = Buffer.concat(centralEntries);
  const eocdBuffer = eocd(files.length, cdBuffer.length, localOffset);

  return Buffer.concat([...localEntries, cdBuffer, eocdBuffer]);
};
...
// ── Vercel deploy via their REST API ──────────────────────────
...
// ── Netlify deploy via their REST API ─────────────────────────
const deployToNetlify = async (
  ...
  if (!NETLIFY_TOKEN) {
    throw new Error("NETLIFY_TOKEN environment variable is not configured");
  }

  const logs: string[] = [];
  const zipBuffer = createZipFromFiles(files);

  // Netlify deploys via site creation or existing site
...
router.post("/deploy/vercel", async (req, res) => {
  ...
  if (!parsed.success) {
    return res.status(400).json({ error: "Invalid deploy data", details: parsed.error.flatten() });
  }
  ...
  try {
    ...
    db.prepare(
      "INSERT INTO deployments (id, userId, projectName, provider, deploymentUrl, status, createdAt) VALUES (?,?,?,?,?,?,?)"
    ).run(id, req.user!.userId, projectName, "vercel", result.url, "live", Date.now());
...
router.post("/deploy/netlify", async (req, res) => {
  ...
  try {
    ...
    db.prepare(
      "INSERT INTO deployments (id, userId, projectName, provider, deploymentUrl, status, createdAt) VALUES (?,?,?,?,?,?,?)"
    ).run(id, req.user!.userId, projectName, "netlify", result.url, "live", Date.now());
...
router.get("/deployments", (req, res) => {
  const rows = db.prepare(
    "SELECT id, projectName, provider, deploymentUrl, status, createdAt FROM deployments WHERE userId = ? ORDER BY createdAt DESC LIMIT 50"
  ).all(req.user!.userId);
  res.json(rows);
});

router.delete("/deployments/:id", (req, res) => {
  db.prepare("DELETE FROM deployments WHERE id = ? AND userId = ?").run(req.params.id, req.user!.userId);
  res.json({ ok: true });
});
